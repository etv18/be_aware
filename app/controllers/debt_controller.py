...
def filter_all():
    try:
        ...
        if start and end:
            ...
            and_filters.append(Debt.created_at.between(start_date, end_date))
        ...
        if query: 
            ...
            if is_cash is not None:
                and_filters.append(Debt.is_cash == is_cash)
            elif is_active is not None:
                and_filters.append(Debt.is_active == is_active)
            else:
                text_filters = db.or_(
                    (Debt.person_name.ilike(q)),
                    (Debt.amount.ilike(q)),
        ...
        debts = (
            Debt.query
            .outerjoin(Debt.bank_account) #allows to show expense without a bank account        
